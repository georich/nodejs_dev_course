    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        Todo.find().then((todos) => {
          expect(todos.length).toBe(2); // 2 accounts for seed data
          done();
        }).catch((e) => done(e));
      });
  });
...
describe('GET /todos', () => {
  it('should get all todos', (done) => {
    request(app)
      ...
      .expect(200)
      .expect((res) => {
        expect(res.body.todos.length).toBe(1); // 1 accounts for userOne having one todo
      })
      .end(done);
  });
});
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User.find().then((users) => {
          expect(users.length).toBe(2); // accounts for seed data
          done();
        }).catch((e) => done(e));
      });
  });
...
describe('DELETE /users/me/token', () => {
  ...
  it('should reject invalid logout request', (done) => {
    request(app)
      .delete('/users/me/token')
